...
class LiveTweet(tweepy.StreamListener):
    def __init__(self, send_msg=False, api=None):
        super().__init__()
    ...
    def on_status(self, status):
        try:
            ...
            if not isRetweet:
                ...
                if self.send_msg and len(data) > 0:
                    self.send_to_kafka(data)
        except Exception as e:
            print(str(e))
            if len(self.item_collection) > 0 and not self.send_msg:
                self.write_to_csv()
                self.print_records()
...
def main():
    ...
    liveTweet = LiveTweet(send_msg=True)
